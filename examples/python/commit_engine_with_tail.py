...
class CommitEngine:
    ...
    def put(self, key: bytes, value: bytes, await_durable: bool = True) -> CommitWindow:
        with self._lock:
            _key_buf, key_ptr, key_len = self._raw(key)
            _value_buf, value_ptr, value_len = self._raw(value)
            window = CommitWindow()
    ...
    def tail_all_merged(
        ...
        while True:
            try:
                ...
                # Current merged replay sometimes reports EOF/idle as operation_failed.
                if "operation_failed (5)" in str(e):
                    items = []
