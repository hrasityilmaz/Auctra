...
pub const AuctraDb = struct {
    allocator: std.mem.Allocator,
    engine: engine_mod.Engine,
    default_durability: types.Durability,
    default_snapshot_kind: types.CheckpointKind,

    pub fn init(allocator: std.mem.Allocator, options: OpenOptions) !AuctraDb {
        return .{
            .allocator = allocator,
    ...
    pub fn deinit(self: *AuctraDb) void {
        self.engine.deinit();
    }

    pub fn commit(self: *AuctraDb) !void {
        try self.engine.commitPending();
    }

    pub fn sync(self: *AuctraDb) !void {
        try self.engine.syncNow();
    }

    pub fn put(self: *AuctraDb, key: []const u8, value: []const u8) !void {
    ...
    pub fn get(self: *AuctraDb, key: []const u8) !ValueView {
        if (key.len == 0) return error.EmptyKey;
    ...
    pub fn exists(self: *AuctraDb, key: []const u8) bool {
        if (key.len == 0) return false;
        return self.engine.get(key) != null;
    }

    pub fn delete(self: *AuctraDb, key: []const u8) !bool {
    ...
    pub fn snapshot(self: *AuctraDb, path: []const u8) !void {
    ...
    pub fn restore(self: *AuctraDb, path: []const u8) !void {
    ...
    pub fn inspectSnapshot(self: *AuctraDb, path: []const u8) !types.SnapshotInspectInfo {
